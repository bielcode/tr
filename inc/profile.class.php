...
/*
   ...
   ------------------------------------------------------------------------
 */
class PluginTrProfile extends Profile
{
    static $rightname = 'profile';
    function rawSearchOptions()
    {
        $tab = [];
        $tab[] = ['id' => 'common', 'name' => __('TR', 'tr')];
        $tab[] = ['id' => '2', 'table' => $this->getTable(), 'field' => 'use', 'linkfield' => 'id', 'datatype' => 'bool'];
        return $tab;
    }
    function showForm($ID, $options = [])
    {
        ...
        $real_right = ProfileRight::getProfileRights($ID, ['plugin_tr']);
        $checked = 0;
        if (isset($real_right) && $real_right['plugin_tr'] == 1) {
            $checked = 1;
        }
        echo "<table class='tab_cadre_fixe'>";
        echo "<tr><th colspan='2' class='center b'>" . __('TR', 'tr');
        echo '</th></tr>';
        echo "<tr class='tab_bg_1'>";
        echo '<td>' . __('TR', 'tr') . '</td><td>';
        Html::showCheckbox(['name' => '_plugin_tr', 'checked' => $checked]);
        echo "</td></tr></table>\n";
        if ($canedit) {
            echo "<div class='center'>";
            echo Html::hidden('id', ['value' => $ID]);
            echo Html::submit(_sx('button', 'Save'), ['name' => 'update', 'class' => 'btn btn-primary', 'icon' => 'ti ti-device-floppy']);
            echo "</div>\n";
            Html::closeForm();
        }
    }
    ...
    {
        self::addDefaultProfileInfos($ID, ['plugin_tr' => 1], true);
    }
    static function getAllRights($all = false)
    {
        $rights = [['itemtype' => 'PluginTr', 'label' => __('TR', 'tr'), 'field' => 'plugin_tr']];
        return $rights;
    }
    static function initProfile()
    {
        ...
        foreach ($DB->request('glpi_profilerights', ['profiles_id' => $_SESSION['glpiactiveprofile']['id'], 'name' => ['LIKE', '%plugin_tr%']]) as $prof) {
    ...
    static function install(Migration $mig)
    {
        ...
        $table = 'glpi_plugin_tr_profiles';
        if (!$DB->tableExists($table) && !$DB->tableExists('glpi_plugin_tr_preferences')) {
            self::initProfile();
            self::createFirstAccess($_SESSION['glpiactiveprofile']['id']);
        } else {
            if ($DB->tableExists($table) && $DB->fieldExists($table, 'ID')) {
