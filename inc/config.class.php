...
class PluginTrConfig extends CommonDBTM
{
    ...
    function getTabNameForItem(CommonGLPI $item, $withtemplate = 0)
    {
        ...
        switch (get_class($item)) {
            case 'Ticket':
                return [1 => __('Termo de Responsabilidade', 'tr')];
            default:
        }
        // add entity tab
        switch (get_class($item)) {
            case 'Entity':
                return [1 => __('Dados para T.R.', 'tr')];
            default:
                return '';
        }
    }
    ...
    function showFormDisplayEntity()
    {
        ...
        // geting rn on db
        if (isset($_GET['id'])) {
            $query_rn = 'SELECT * FROM glpi_plugin_tr_rn WHERE entities_id = ' . $_GET['id'];
        ...
        if ($canedit) {
            echo "<form name='form' action='../plugins/tr/front/insert_rn.php' method='post'>";
        }
        echo '<td>' . __('CNPJ: ') . '</td>';
